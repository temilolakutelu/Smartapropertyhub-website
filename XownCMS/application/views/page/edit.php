...
<div class="panel panel-default form-panel" data-widget='{"draggable": "false"}'>
    <div class="panel-heading">
        <h2>Update <?= $name; ?> web page</h2>
    </div>
    <div class="panel-body">
       
        <form action="<?= site_url('page/update_action'); ?>" method="post" enctype="multipart/form-data">    
             <div class="form-group col-md-5">
                ...
                <select type="text" class="region_id form-control" name="category_id" id="region_id" required>
                    ...
                    foreach ($category_data as $region) { ?>
                            <option value="<?= $region->categoryID ?>" <?php if ($region->categoryID == $page_id) {
                                                                            echo 'selected';
                                                                        } ?> ><?= $region->categoryName ?></option>
                    <?php 
                }
                ?>
                </select>
    ...
    <input type="hidden"  name="id" value="<?= $id; ?>">
                ...
                <input type="text" class="form-control" name="name" id="cus_id" value="<?php echo $name; ?>" pattern="[a-zA-Z0-9\s]+" title="Only Alphanemeric allowed"/>
                
            </div>
            <div class="form-group col-md-5">
                <label for="staff_no">Meta Data</label>
                <input type="text" class="form-control" name="meta_data" i placeholder="Meta data" value="<?php echo $meta_data; ?>"/>
            </div>
           ...
           <div class="form-group col-md-5">
            ...
            <div class="form-group col-md-10">
                <label for="health_history">Content<span class="required">*</span></label>
                <textarea class="form-control autosize" rows="3" name="content" id="summernote"><?php echo $content; ?></textarea>
            </div>
           
        <div class="form-group col-md-3">&nbsp;
       </div>
        <input type="submit" class="finish btn-success btn form-group col-md-3" value="Update Webpage" />
        </form>
    </div>
</div>
