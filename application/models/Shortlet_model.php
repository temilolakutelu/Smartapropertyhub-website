...
class Shortlet_model extends CI_Model
{
...
function get_shortlet_state(){
    $this->db->select('state');
     $this->db->from('tbl_properties');
     $this->db->distinct();
     $this->db->where('tbl_properties.category_ID =', 3)
...
$this->db->where('tbl_property_images.imageStatus', 'active');
   ...
   public function record_count() 
   {
        ...
        return $this->db->where('tbl_properties.category_ID',3)
            ->where('status_Details', 'published')
