...
//To get the number of properties that can be fetched with the search query
	...
	"WHERE category_ID = 1 LIMIT $start, $display";
...
else
{
	...
	echo'<br /><a href="user/index.php" id="saveSearch" class="search"> SAVE SEARCH &#9654 </a><br/><br/>';
	if ($pages > 1) 
	{
	...
	while($row = mysql_fetch_array($result))
	{
		...
		echo $saleDetails = '<div class="propListing">'.
			...
			$row['property_Name'].'</font></p><i class="fa fa-map-marker"></i>'.
			$row['property_Street'].', '.
			$row['property_City'].'. '.
			$row['property_State'].'. '.
