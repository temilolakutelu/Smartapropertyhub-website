...
<div id="salesRent">PROPERTIES FOR SALE</div>
...
//To get the number of properties that can be fetched with the search query
	...
	" p.property_Price AS property_Price, p.status_Details AS status, p.property_Description AS property_Description, p.currency AS pCurrency,".
	...
	"WHERE category_ID = 1  GROUP BY p.property_ID ORDER BY p.property_ID DESC LIMIT $start, $display";
...
else
{
	...
	while($row = mysql_fetch_array($result))
	{
		...
		echo $saleDetails = '<div class="propListing">'.
			'<div class="propImg left"><a href="propertyDetails.php?prop='.$propID.'"><img src="smartpro/uploads/photos/'.$imgPath.'" alt="image" /></a>
			<div class="camera_div"><i class="fa fa-camera"></i><font class="num_font">'.getNumOfImage($propID).'</font></div>
			'.$status.'
			</div><div class="propDet left"><font class="pricing">'.$currencyQuote.'</font>
			<span class="pricing">'.
...
<div id="sticky-anchor"></div>
		...
		<div class="advertorial">
        	<div class="advertorial_inner" id="sticky">
				<script type="text/javascript" src="http://konga.postaffiliatepro.com/scripts/banner.php?k_id=4080&k_bid=562ad577&chan=2206"></script>
            </div>
        </div>
